...
import { TroikaText } from '~/text/js/webgl/TroikaText';
...
export class WebGL {
  constructor({ canvasWrapper, canvas, selfLoop = true }) {
    ...
    this.text = new TroikaText({ viewSize });
  ...
  ticker = () => {
    ...
    this.glCanvas.offscreenRender({
      renderTarget: this.text.fbo,
      scene: this.text.scene,
      camera: this.camera,
    });

    this.screen.update({ time, texture: this.text.fbo.texture });
